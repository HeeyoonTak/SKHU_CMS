								...
								<c:forEach var="user" items="${acceptanceNo}">
									<tr>
										<td><input type="hidden" name="user_id"
											value="${userClub.user_id}" /> <input type="hidden"
											name="club_id" value="${userClub.club_id}" /> <label><input
												type="checkbox"
												style="margin-right: 10px; width: 18px; height: 18px;"
												name="cmd" value="yes">${user.name}</label> <i
											class="btn btn-primary el-icon-document"
											style="margin-left: 78.5px" data-target="#formModal"
											onclick="return showForm('${apply_a.club_id}')"></i></td>
									</tr>
								</c:forEach>
							</table>
						</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="overflow: scroll">
				<table class="table table-striped ">
					...
					<tr>
						<td style="margin-right: 10px; width: 18px; height: 18px; font-weight:bold">질문   :</td>
					...
					<tr>
						<td style="margin-right: 10px; width: 18px; height: 18px; font-weight:bold"">답변   :</td>
