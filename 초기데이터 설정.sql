use softwarecapstion;

SET FOREIGN_KEY_CHECKS = 0; -- Disable foreign key checking. 
TRUNCATE TABLE club_type; 
truncate table club;
truncate table account;
truncate table user;
truncate table attendance;
truncate table board;
truncate table board_name;
truncate table sem_date;
truncate table files;
truncate table user_club;
truncate table apply_q;
truncate table apply_a;
SET FOREIGN_KEY_CHECKS = 1; -- Enable foreign key checking.

ALTER TABLE `softwarecapstion`.`apply_a` 
DROP FOREIGN KEY `FK_applyA_userId`;
ALTER TABLE `softwarecapstion`.`apply_a` 
ADD CONSTRAINT `FK_applyA_userId`
  FOREIGN KEY (`user_id`)
  REFERENCES `softwarecapstion`.`user` (`id`)
  ON DELETE CASCADE;

ALTER TABLE `softwarecapstion`.`attendance` 
DROP FOREIGN KEY `FK_attendance_userId`;
ALTER TABLE `softwarecapstion`.`attendance` 
ADD CONSTRAINT `FK_attendance_userId`
	FOREIGN KEY (`user_id`)
	REFERENCES `softwarecapstion`.`user` (`id`)
	ON DELETE CASCADE;

-- 동아리 타입
INSERT INTO club_type (id,type_name) VALUES
	(1,'동아리연합회'),
	(2,'종교/학술'),
	(3,'문화기획체육'),
	(4,'공연1'),
	(5,'공연2') ; 

-- 동아리
INSERT INTO club (id,club_name, club_type, content) VALUES
	(1,'동아리연합회', 1, '모든 동아리의 행사와 복지를 관리, 지원하는 학생 자치기구'),
	(2,'멋쟁이사자처럼', 2, '비전공자/전공기초를 위한 웹 프로그래밍 개발 동아리'),
	(3,'FLOW', 3, '농구를 사랑하는 사람들의 모임'),
	(4,'ELPIS', 4, '밴드 동아리'),
	(5,'아침햇살', 5, '춤 동아리');

-- 유저 입력
INSERT INTO user (id, name, login_id, password, user_type, phone, email) VALUES
				(1, '동아리연합회', 'clubAs', '12341234', '동연', null, null),
				(2, '멋쟁이사자처럼', 'Lion', '12341234', '동아리관리자', null, null),
				(3, 'FLOW', 'Flow', '12341234', '동아리관리자', null, null),
                (4, 'ELPIS', 'Elpis', '12341234', '동아리관리자', null, null),
                (5, '아침햇살', 'Rice', '12341234', '동아리관리자', null, null),
                (6, '탁희윤', '201332039', '12341234', '일반회원', null, null),
                (7, '김명석', '201433006', '12341234', '일반회원', null, null),
                (8, '이혜민', '201532028', '12341234', '일반회원', null, null),
                (9, '안세연', '201632012', '12341234', '일반회원', null, null),
                (10, '장예지', '201632027', '12341234', '일반회원', null, null);

-- 유저와 동아리 연결
LOCK TABLES `user_club` WRITE;
/*!40000 ALTER TABLE `user_club` DISABLE KEYS */;
INSERT INTO user_club (id, user_id, club_id) VALUES 
	(1,1,1),
	(2,2,2),
	(3,3,3),
	(4,4,4),
	(5,5,5),
    (6,7,2),
    (7,8,2);
/*!40000 ALTER TABLE `user_club` ENABLE KEYS */;
UNLOCK TABLES;

-- 회계 입력
INSERT INTO account (id, club_id, price, total, remark, account_type, date) VALUES
					(1,1, 200000, 200000, null,  0, '2020-03-02'),
                    (2,1, -200000, 0, null,  0, '2020-03-03'),
                    (3,1, 100000, 100000, null, 1, '2020-03-07'),
                    (4,1, -50000, 50000, null, 1, '2020-03-10'),
                    (6,2, -200000, 0, null, 0, '2020-03-02'),
                    (8,3, -200000, 0, null, 0, '2020-03-10'),
                    (9,4, 200000, 200000, null, 0, '2020-03-02'),
                    (10,4, -200000, 0, null, 0, '2020-03-02'),
                    (11,1, 200000, 250000, null, 0, '2020-10-10') ;

-- 출석 입력
-- INSERT INTO attendance (id, club_id, `check`, date, user_id) VALUES
--   (1, 1, 0, '2020-03-03', 2),
--   (2, 1, 1, '2020-03-03', 3),
--   (3, 1, 1, '2020-03-03', 4),
--   (4, 1, 1, '2020-03-03', 5),
--   (5, 1, 1, '2020-03-10', 2),
--   (6, 1, 1, '2020-03-10', 3),
--   (7, 1, 1, '2020-03-10', 4),
--   (8, 1, 1, '2020-03-10', 5),
--   (9, 1, 1, '2020-03-17', 2),
--   (10, 1, 0, '2020-03-17', 3),
--   (11, 1, 1, '2020-03-17', 4),
--   (12, 1, 0, '2020-03-17', 5),
--   (13, 1, 1, '2020-03-24', 2),
--   (14, 1, 1, '2020-03-24', 3),
--   (15, 1, 0, '2020-03-24', 4),
--   (16, 1, 1, '2020-03-24', 5),
--   (17, 1, 1, '2020-03-31', 2),
--   (18, 1, 1, '2020-03-31', 3),
--   (19, 1, 1, '2020-03-31', 4),
--   (20, 1, 1, '2020-03-31', 5),
--   (21, 1, 1, '2020-04-07', 2),
--   (22, 1, 1, '2020-04-07', 3),
--   (23, 1, 0, '2020-04-07', 4),
--   (24, 1, 1, '2020-04-07', 5),
--   (25, 1, 1, '2020-04-14', 2),
--   (26, 1, 1, '2020-04-14', 3),
--   (27, 1, 1, '2020-04-14', 4),
--   (28, 1, 1, '2020-04-14', 5),
--   (29, 1, 0, '2020-09-01', 2),
--   (30, 1, 1, '2020-09-01', 3),
--   (31, 1, 0, '2020-09-01', 4),
--   (32, 1, 1, '2020-09-01', 5),
--   (33, 1, 1, '2020-09-01', 11),
--   (34, 1, 0, '2020-09-01', 12),
--   (35, 1, 1, '2020-09-08', 2),
--   (36, 1, 1, '2020-09-08', 3),
--   (37, 1, 1, '2020-09-08', 4),
--   (38, 1, 0, '2020-09-08', 5),
--   (39, 1, 1, '2020-09-08', 11),
--   (40, 1, 0, '2020-09-08', 12),
--   (41, 1, 1, '2020-09-15', 2),
--   (42, 1, 0, '2020-09-15', 3),
--   (43, 1, 1, '2020-09-15', 4),
--   (44, 1, 1, '2020-09-15', 5),
--   (45, 1, 0, '2020-09-15', 11),
--   (46, 1, 0, '2020-09-15', 12),
--   (47, 1, 1, '2019-03-10', 2),
--   (48, 1, 1, '2019-03-10', 3),
--   (49, 1, 1, '2019-04-16', 2),
--   (50, 1, 0, '2019-04-16', 3),
--   (51, 1, 1, '2019-05-31', 2),
--   (52, 1, 1, '2019-05-31', 3),
--   (53, 1, 1, '2019-06-21', 2),
--   (54, 1, 1, '2019-06-21', 3),
--   (55, 1, 0, '2019-07-12', 2),
--   (56, 1, 1, '2019-07-12', 3),
--   (57, 1, 1, '2019-08-31', 2),
--   (58, 1, 1, '2019-08-31', 3),
--   (59, 1, 1, '2019-08-31', 4),
--   (60, 1, 1, '2019-09-25', 2),
--   (61, 1, 1, '2019-09-25', 3),
--   (62, 1, 1, '2019-09-25', 4),
--   (63, 1, 1, '2019-10-01', 2),
--   (64, 1, 1, '2019-10-01', 3),
--   (65, 1, 1, '2019-10-01', 4),
--   (66, 1, 0, '2019-11-14', 2),
--   (67, 1, 1, '2019-11-14', 3),
--   (68, 1, 0, '2019-11-14', 4),
--   (69, 1, 0, '2019-12-25', 2),
--   (70, 1, 1, '2019-12-25', 3),
--   (71, 1, 0, '2019-12-25', 4),
--   (72, 1, 1, '2020-01-12', 2),
--   (73, 1, 0, '2020-01-12', 3),
--   (74, 1, 1, '2020-01-12', 4),
--   (75, 7, 1, '2020-03-10', 10),
--   (76, 7, 1, '2020-03-10', 14),
--   (77, 7, 1, '2020-03-10', 15),
--   (78, 7, 1, '2020-05-03', 10),
--   (79, 7, 1, '2020-05-03', 14),
--   (80, 7, 1, '2020-05-03', 15),
--   (81, 7, 1, '2020-05-31', 10),
--   (82, 7, 1, '2020-05-31', 14),
--   (83, 7, 1, '2020-05-31', 15),
--   (84, 7, 1, '2019-08-31', 10),
--   (85, 7, 1, '2019-08-31', 14),
--   (86, 7, 1, '2019-08-31', 15),
--   (87, 7, 1, '2019-10-04', 10),
--   (88, 7, 1, '2019-10-04', 14),
--   (89, 7, 1, '2019-10-04', 15),
--   (90, 7, 1, '2019-11-24', 10),
--   (91, 7, 1, '2019-11-24', 14),
--   (92, 7, 1, '2019-11-24', 15);

-- 게시판 타입 
INSERT INTO board_name (id, board_name) VALUES
	(1, '홍보'),
    (2, '모집'),
    (3, '공지'),
    (4, '회의록');

-- 게시글 입력
 INSERT INTO board (id, board_name_id, title, content, date, club_id, start_date, end_date) VALUES
					-- 홍보 게시판
					(1, 1, '[멋사] 아이디어톤 모집', '10월 30일에 아이디어톤을 지원받습니다~!', '2020-10-06', 2, null, null),
                    (2, 1, '[ELPIS] 공연 날짜 안내', '자연드립 앞 공터에서 10월 23일 공연합니다 ^^!', '2020-10-3', 4, null, null),
                    (3, 1, '[FLOW] 20년 2학기 농구대회', '플로우가 주최하는 농구대회! 11월 2일에 농구장에서 열립니다', '2020-10-7', 3, null, null),
                    -- 모집 게시판
                    (4, 2, '[멋사] 8.5기 모집', '8.5기 모집해요!.', '2020-09-27', 2, '2020-10-02', '2020-10-28'),
                    (5, 2, '[아침햇살] 모집', '아침햇살 모집합니다.', '2020-04-10', 5, '2020-04-12', '2020-04-21'),
                    -- 공지 게시판
                    (6, 3, '[동연] 추석이벤트 참여!', '추석을 맞아 이벤트합니다!', '2020-09-28',3,null,null),       
                    (7, 3, '멋사 공지사항', '공지사항 알립니다.', '2020-09-20', 2, null, null),
                    -- 회의록
                    (10, 4, '[동연] 2학기 1차 회의록', '회의록~~~~~~', '2020-09-10', 1, null, null),
                    (11, 4, '1차 회의록', '멋사 회의록', '2020-09-12', 2, null, null);
                    
  
INSERT INTO sem_date (id, start_date, end_date, sem_name) VALUES
						(1, '2020-03-01', '2020-08-31', '2020-1학기'),
                        (2, '2020-09-01', '2021-02-28', '2020-2학기'),
                        (3, '2019-03-04', '2019-08-25', '2019-1학기'),
                        (4, '2019-08-26', '2020-02-29', '2019-2학기');
 
INSERT INTO attendance VALUES
						(1, 1, 1, '2020-10-01', 2),
                        (2, 1, 1, '2020-10-01', 3),
                        (3, 1, 1, '2020-10-01', 4),
                        (4, 1, 1, '2020-10-01', 5),
					    (5, 2, 1, '2020-09-28', 7),
                        (6, 2, 1, '2020-09-28', 8);
                        
INSERT INTO apply_q (id, content, board_id, club_id, semDate_id) VALUES 
						('1', '왜 지원하셨나요?', '2', '2', 2),	
                        ('2', '이전의 동아리 경력이 있으신가요?', '2', '2', 2),
                        ('3', '자신을 1문장으로 표현한다면?', '2', '2', 2);

                        
                        
                        
-- INSERT INTO apply_a (id,apply_q_id, content, user_id, club_id) VALUES 
-- 									('1','1', '심심해서요', '13', '2'), 
--                                     ('2','2', '56', '13', '2'),
--                                     ('3','3', '죽지못해 산다', '13', '2'),
--                                     ('4','1', 'ㄲ', '16', '2'),
--                                     ('5','2', 'ㄸ', '16', '2'),
--                                     ('6','3', 'ㅉ 산다', '16', '2');
